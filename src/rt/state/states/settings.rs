...
impl Settings {
    pub fn new () -> Self {
        Self {
            controllers: match get_controllers() { Ok(controllers) => controllers, Err(err) => panic!("{}", err)},
            state: SubState::Main,
            cursor_pos: (0, 0),
            inputmap: vec![],
        }
    ...
    fn action (&mut self) -> Option<GameEvent> {
        self.state = SubState::KeySelect;
        None
    }

    // switched between tabs
    fn tab (&mut self, dir: i8) -> Option<GameEvent> {
        let new = ((self.cursor_pos.1 as i8 + dir) + 3) as usize % 3;
        if new != self.cursor_pos.1 {
            self.cursor_pos.1 = new;
        }
        None
    }
    ...
    fn scroll (&mut self, dir: i8) -> Option<GameEvent> {
        self.cursor_pos.0 = ((self.cursor_pos.0 as i8 + dir) + 8) as usize % 8;
        None
    }
...
impl RustyEngine for Settings {
    ...
    fn render(&mut self, api: &mut dyn doryen_rs::DoryenApi) {
        ...
        // render title
        let title = format!("Controller Settings: {}", ["Singleplayer", "Versus: Player 1", "Versus: Player 2"][self.cursor_pos.1]);
        render_button(con, 0, 0, CONSOLE_WIDTH, 5, title.as_str(), white, Some(darker_gray), None, (Align::Start, Align::Start));
        ...
        if category == "selk" {
            ...
            for i in 0..valid_keys.len() {
                if input.key_pressed(valid_keys[i]) {
                    ...
                    self.controllers[self.cursor_pos.1].set_at(self.cursor_pos.0, valid_keys[i]);
                    self.state = SubState::Main;
                    return None
                }
        ...
        for index in 0..self.inputmap.len() {
            ...
            if self.inputmap[index].trigger(input).to_owned() {
                match (self.inputmap[index].key_text.as_str(), self.inputmap[index].category.as_str()) {
                    ...
                    ("ArrowLeft", "main")     => return self.tab(-1),
                    ("ArrowRight", "main")    => return self.tab(1),
